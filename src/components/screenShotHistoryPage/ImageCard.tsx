...
const ImageCard = ({ imgSrc, title }: { imgSrc: string; title: string }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg group">
      ...
      >
        ...
        <div className="absolute inset-0 bg-black bg-opacity-50 p-4 rounded-[16px] flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out">
          ...
          <div className="flex absolute bottom-4 justify-between w-full pr-6">
            <span className="rounded-full py-[6px] px-4 bg-[#e5e5e5] font-semibold cursor-pointer text-sm flex items-center justify-center hover:bg-white">
              twitter.com
            </span>
            <div className="flex items-center space-x-2">
              <Button
                variant="secondary"
                className="rounded-full h-9 w-9 flex items-center justify-center bg-[#e5e5e5] hover:bg-white border-none shrink-0"
              >
                <span>
                  <LuUpload size={18} />
                </span>
      ...
      <p className="mt-2 font-bold ml-2 text-[18px]">{title}</p>
        ...
        <AlertDialogContent className="max-w-lg w-full h-auto max-h-[80vh] bg-transparent text-white shadow-none outline-none border-none">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center justify-between">
              {title}
              <span
                className="bg-white hover:bg-[#e5e5e5] text-black h-9 w-9 flex items-center justify-center rounded-full cursor-pointer transition duration-300"
